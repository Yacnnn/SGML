import numpy as np
import ot
from utils.process import uniform_transport_matrix, uniform_part_pw
...
def rpswv(distrib1, distrib2):
    ...
    argsort_distrib1 = np.argsort(distrib1,axis = 0)
    argsort_distrib2 = np.argsort(distrib2,axis = 0)
    c = [uniform_part_pw(len(distrib1),len(distrib2),distrib1[argsort_distrib1[:, t],:],distrib2[argsort_distrib2[:, t],:]) for t in range(d)]
    return np.sqrt(np.sum(c)/d)

@timeis
def pswv(distrib1, distrib2):
    d = distrib1.shape[1]
    T = np.random.normal(size= (d, 50))
...
if __name__ == '__main__':
    # n_samples_list = np.logspace(1, 6, num=18, endpoint=True, base=10.0, dtype=int, axis=0)
    n_samples_list = np.logspace(1, 8, num=18, endpoint=True, base=10.0, dtype=int, axis=0)
    C = np.zeros((7,len(n_samples_list)))
    C[6,:] = n_samples_list
    for i, n_samples in enumerate(n_samples_list):
        ...
        if C[2,i] == 0 :#nd n_samples < 6812925 :#and n_samples < 6812925
        ...
        if C[5,i] == 0 :
            C[5,i] = pswv(distrib1, distrib2)[-1]
        print(i,n_samples)
